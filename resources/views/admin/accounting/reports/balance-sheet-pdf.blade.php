    ...
    <table class="brand">
        <tr>
            ...
            <td style="text-align:left;">
                ...
                <div class="subtitle">حتى تاريخ: {{ $filters['date_to'] ?: date('Y-m-d') }}</div>
...
<main>
    <table class="report">
        <thead><tr><th colspan="3">الأصول</th></tr><tr><th>الكود</th><th>الحساب</th><th>الرصيد</th></tr></thead>
        <tbody>
        @foreach($assets as $r)
            <tr><td>{{ $r->code }}</td><td>{{ $r->name }}</td><td>{{ number_format($r->balance, 2) }}</td></tr>
        @endforeach
        <tr><th colspan="2">إجمالي الأصول</th><th>{{ number_format($assetTotal, 2) }}</th></tr>
        </tbody>
    </table>

    <table class="report">
        <thead><tr><th colspan="3">الالتزامات وحقوق الملكية</th></tr><tr><th>الكود</th><th>الحساب</th><th>الرصيد</th></tr></thead>
        <tbody>
